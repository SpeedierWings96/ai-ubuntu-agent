...
function Show-Banner {
    ...
    Write-Host "==========================================" -ForegroundColor Blue
    Write-Host ""
    Write-Info  "This will set up an isolated Ubuntu desktop"
    Write-Info  "running in a Docker container on your machine."
    Write-Host  ""
}
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if (-not $NonInteractive) {
                ...
                docker info 2>&1 | Out-Null
                if ($LASTEXITCODE -ne 0) { throw "Docker Desktop is still not running" }
            }
            else {
        ...
        # Check WSL2 presence (best-effort)
        Write-Info "Checking WSL2..."
        $null = wsl --status 2>$null
        if ($LASTEXITCODE -eq 0) {
            Write-Success "✓ WSL2 is installed"
        } else {
            Write-Warning "WSL2 might not be properly configured (Docker typically sets this up)"
        }
    }
    catch {
        Write-Err "Docker Desktop is not installed or not accessible"
        Write-Info ""
...
function New-EnvFile {
    ...
    if (Test-Path env.example) {
        ...
        Write-Err "env.example file not found"
        exit 1
    }
    ...
    if (-not $NonInteractive) {
        ...
        if ($apiKey) {
            ...
            Write-Warning "No API key provided. You will need to update .env manually."
        }

        Write-Host ""
        $vncPrompt = "Enter VNC Password (press Enter for default: changeme)"
        $vncPass = Read-Host $vncPrompt
...
function Start-Services {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed to build containers" }
    ...
    while ($attempt -lt $maxAttempts) {
        try {
            ...
            if ($response.StatusCode -eq 200) { Write-Success "✓ Agent service is ready"; break }
        }
        catch {
            $attempt++
